...
def is_playlist(url: str) -> list:
    """Function that verify if the video linked is in a playlist and return 
...
def get_video_from_playlist(build: object, playlist_id: str) -> list:
    """Function that returns a list with the title and id of each video in the playlist corresponds to the parameter playlist_id.
...
def path_exist() -> str:
    """Function that ask a path and verify that it exist.
...
def download_progress_info(dico: dict):
    """Callback function to get the progress information of the current download.
...
def download_video_mp3(video_id: str, path: str):
    """Function that download the mp3 files from a ytb video.
